...
module.exports = function getAPIDatas() {
  ...
  function getConsumption(session, deviceTypeConso, deviceTypePower, minDayConso, minDayPower, maxDay) {
      ...
      .then((result) => {
        ...
        return Promise.map(result.data, function (data, index) {
          ...
          .then(() => {
            ...
            getMaxPower(session, deviceTypePower, minDayPower, maxDay);
          })
          .catch((error) => {
            sails.log.error(error);
          });
  ...
  function getMaxPower(session, deviceTypePower, minDay, maxDay) {
    if (minDay == maxDay) {
      // La date minimum et maximum étant les mêmes, rien à récupérer
      return;
    }
  ...
  sails.log.info(`Linky : getAPIConsomDay`);
  try {
      ...
      .then(([devicesTypeSelectConsoDay, devicesTypeSelectMaxPower]) => {
        ...
        Promise.all([
          ...
          .then(([deviceStatesDay, deviceStatesPower]) => {
            ...
            getConsumption(session, deviceTypeConso, deviceTypePower, minDayConso, minDayPower, now);
          })
          .catch((error) => {
            sails.log.error(error);
          });
